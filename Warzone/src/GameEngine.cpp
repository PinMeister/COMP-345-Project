...
void GameEngine::executeOrdersPhase(PhaseObserver *phaseObserver)
{
    ...
    {                                            //iterating through list of players
        ...
        for (iter = playerOrders.begin(); iter != playerOrders.end(); iter++)
        { //iterating through each player's list of orders
            if (typeid(*iter) != typeid(Deploy))
            { //skips iteration if not deploy
                continue;
            }
            (*iter)->execute();
        }
        for (iter = playerOrders.begin(); iter != playerOrders.end(); iter++)
        { //iterating through each player's list of orders
            if (typeid(*iter) != typeid(Airlift))
            { //skips iteration if not airlift
                continue;
            }
            (*iter)->execute();
        }
        for (iter = playerOrders.begin(); iter != playerOrders.end(); iter++)
        { //iterating through each player's list of orders
            if (typeid(*iter) != typeid(Blockade))
            { //skips iteration if not blockade
                continue;
            }
            (*iter)->execute();
        }
